...
def addSubdomain(conn, domain, pe_org_uid, org_name, thread):
...
def run_dnstwist(orgs, thread):
    ...
    for org_index, org_row in orgs.iterrows():
        ...
        try:
            ...
            LOGGER.error(f"{thread}: Failed connecting to PE database.")
        ...
        try:
            ...
            for rd_index, rd_row in rd_df.iterrows():
                ...
                LOGGER.info("%s:\t\t%s", thread, sub_domain)
                try:
                    sub_domain_uid = getSubdomain(PE_conn, sub_domain)[0]
                    # LOGGER.info(sub_domain_uid)
                except:
                    # Add and then get it
                    addSubdomain(PE_conn, sub_domain, pe_org_uid, org_name, thread)
                    sub_domain_uid = getSubdomain(PE_conn, sub_domain)[0]
            ...
            LOGGER.error(f"{thread}:Failed selecting DNSTwist data.")
...
def main():
    ...
    # Get P&E orgs
    org_df = query_orgs_rev()
    LOGGER.info(org_df["name"])
    LOGGER.info(org_df["cyhy_db_name"].tolist())

    orgs_list = np.array_split(org_df, 4)
    thread_list = []
    x = 0
    for org in orgs_list:
        thread_name = f"DNSTwist Thread {x+1}: "
        # Start thread
        t = threading.Thread(
            target=run_dnstwist, args=(org, thread_name)
        )
        t.start()
