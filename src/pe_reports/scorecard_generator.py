...
def determine_arrow(value, last_value, color=False):
    ...
    if color:
        if value_diff > 0:
            return BASE_DIR + "/assets_scorecard/up_red.png"
        elif value_diff < 0:
            return BASE_DIR + "/assets_scorecard/down_green.png"
        else:
            return BASE_DIR + "/assets_scorecard/no_change.png"
    else:
        if value_diff > 0:
            return BASE_DIR + "/assets_scorecard/up_black.png"
        elif value_diff < 0:
            return BASE_DIR + "/assets_scorecard/down_black.png"
        else:
            return BASE_DIR + "/assets_scorecard/no_change.png"
...
def create_scorecard(data_dict, file_name):
    ...
    if isinstance(data_dict["circles_df"], pd.DataFrame):
        ...
        can.drawImage(
            BASE_DIR + "/assets_scorecard/circle_line.png", 0, 18, 255, 255, mask="auto"
        )
...
def generate_circle_chart(df, name):
    ...
    plt.savefig(
        BASE_DIR + "/assets_scorecard/" + name,
        transparent=False,
        dpi=500,
        bbox_inches="tight",
    )
