...
def sendIPS(ip_list: list) -> str:
    """Send a list of domains to WhoisXML API and retuern the requestID."""
...
def getDomains(requestId: str, index: int, blockSize: int) -> list:
    """Fetch domains from whoisXML using requestid and returns a list of blockSize amount of records."""
    ...
    if responseJson["recordsProcessed"] < min(
        index + blockSize, responseJson["totalRecords"]
    ):
        print(
