...
class UpdateWeeklyStatusesForm(forms.Form):
    ...
    updatetheuserIssues = forms.ChoiceField(label='', choices=[],
                                      required=True,
                                      widget=forms.Select(
                                          attrs={'id': 'updatetheuserIssues'}))
    ...
    updateongoing_task = forms.CharField(label='Ongoing Tasks',
                                   widget=forms.Textarea(attrs={"rows": "2"}))


    updateupcoming_task = forms.CharField(label='Upcoming Tasks',
                                    widget=forms.Textarea(attrs={"rows": "2"}))

    updateobstacles = forms.CharField(label='Obstacles or Active Blockers',
                                widget=forms.Textarea(attrs={"rows": "2"}))

    updatenon_standard_meeting = forms.CharField(label='Non-standard meetings',
                                           widget=forms.Textarea(attrs={"rows": "2"}))

    updatedeliverables = forms.CharField(label='Deliverables',
                                   widget=forms.Textarea(attrs={"rows": "2"}))

    updatepto_time = forms.CharField(label='Upcoming PTO',
                               widget=forms.Textarea(attrs={"rows": "2"}))
        ...
        if self.current_user and self.current_user.is_authenticated:
            ...
            if user_issues_dict:
                ...
                messages.warning(request,
                                 "The user has not been assigned any issues. ")
    ...
    def save(self, user):
        if self.is_valid():
            weekly_status = WeeklyStatuses.objects.filter(
                user_status=user.first_name).latest('week_ending')
            # update the fields of weekly_status from self.cleaned_data
            weekly_status.key_accomplishments = self.cleaned_data.get(
                'updatekey_accomplishments')
            weekly_status.ongoing_task = self.cleaned_data.get(
                'updateongoing_task')
            weekly_status.upcoming_task = self.cleaned_data.get(
                'updateupcoming_task')
            ...
            weekly_status.deliverables = self.cleaned_data.get(
                'updatedeliverables')
