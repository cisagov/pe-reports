...
def login():
    ...
    try:
        if form.validate_on_submit():
            ...
            LOGGER.info("The user %s was entered.", user)
            if user.check_password(form.password.data) and user is not None:

                login_user(user)
                flash(f'{user.username} you have logged in successfully!',
                      'success')

                nexty = request.args.get('next')

                if nexty is None or not nexty[0] == '/':
                    nexty = url_for('home.index')

                return redirect(nexty)
    except AttributeError:
        flash('The user name or password that'
              ' you have entered is incorrect, please try again.', 'warning')
